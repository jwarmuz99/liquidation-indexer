...
describe("InitializableImmutableAdminUpgradeabilityProxy contract LiquidationCall event tests", () => {
  ...
  const event = InitializableImmutableAdminUpgradeabilityProxy.LiquidationCall.createMockEvent({/* It mocks event fields with default values. You can overwrite them if you need */});

  it("InitializableImmutableAdminUpgradeabilityProxy_LiquidationCall is created correctly", async () => {
    // Processing the event
    const mockDbUpdated = await InitializableImmutableAdminUpgradeabilityProxy.LiquidationCall.processEvent({
      event,
      mockDb,
    });

    // Getting the actual entity from the mock database
    let actualInitializableImmutableAdminUpgradeabilityProxyLiquidationCall = mockDbUpdated.entities.InitializableImmutableAdminUpgradeabilityProxy_LiquidationCall.get(
